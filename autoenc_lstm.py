...
class LstmAutoencoder(Model):
    ...
    def __init__(self,
        ...
        self.hiddenlayer1 = torch.nn.Linear(in_features=encoder.get_output_dim(),
                                          out_features=200)
        self.decoder = torch.nn.Linear(in_features=200, out_features=1024)
    ...
    def forward(self,
        ...
        output["loss"] = self.loss(decoded, embeddings)
          
        return output

    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        return {"accuracy": torch.tensor(0) }
...
train_dataset = reader.read(cached_path('csv/unlabeled_70k.csv'))
...
model= LstmAutoencoder(word_embeddings, lstm, vocab)
