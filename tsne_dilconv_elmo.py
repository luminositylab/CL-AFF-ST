...
class BigramDilatedConvModel(Model):
    ...
    def forward(self,
        ...
        #tensor_list = [hidden_representation]
        #self.tsne_x = torch.stack(tensor_list)
        #self.tsne_x = self.tsne_x[-1, :, :]
        #print(self.tsne_x.shape)
        #social_sq = social.unsqueeze(1)
        #agency_sq = agency.unsqueeze(1)
        tsne_x = hidden_representation
        ...
        #print(labels)
        for item in t_y:
            if item[0] == social_agency[0] and item[1] == social_agency[1]:
                tsne_y.append(0)
            elif item[0] == social_only[0] and item[1] == social_only[1]:
                tsne_y.append(1)
            elif item[0] == agency_only[0] and item[1] == agency_only[1]:
                tsne_y.append(2)
            elif item[0] == not_any[0] and item[1] == not_any[1]:
                tsne_y.append(3)
        tsne_y = np.asarray(tsne_y)
        #print(tsne_y)


        X_2d = tsne.fit_transform(tsne_x.detach().cpu().numpy())
